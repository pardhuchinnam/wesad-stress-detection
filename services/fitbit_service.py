...
class FitbitDataService:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            try:
                ...
                logger.error("The 'fitbit' package is not installed. Run: pip install fitbit")
                return False
    ...
    def get_heart_rate_intraday(self, date=None):
        ...
        try:
            ...
            def fetch():
                data = self.client.intraday_time_series('activities/heart', base_date=date, detail_level='1min')
    ...
    def get_current_heart_rate(self):
        ...
        try:
            ...
            if hr_data and len(hr_data) > 0:
                return hr_data[-1]['value']
            return None
        except Exception as e:
            logger.error(f"❌ Current heart rate fetch failed: {e}")
            return None
        ...
        try:
            ...
            def fetch():
                ...
                summary = {
                    ...
                    'distance': 0,
                    'active_minutes': data['summary'].get('fairlyActiveMinutes', 0) + data['summary'].get('veryActiveMinutes', 0),
                ...
                distances = data['summary'].get('distances', [])
                if distances:
                    summary['distance'] = distances[0].get('distance', 0)
    ...
    def stream_physiological_data(self):
        ...
        try:
            ...
            latest_hr = hr_data[-1]['value'] if hr_data else 70
            ...
            logger.info(f"✅ Physiological data streamed: HR={latest_hr}, Steps={activity.get('steps', 0) if activity else 0}")
    ...
    def get_stress_score_from_hrv(self):
        """Calculate stress score based on HRV (higher HRV = lower stress)"""
        try:
            ...
            # Normalize typical RMSSD range (20-100 ms) to stress score (0 to 1)
            if rmssd < 20:
                stress_score = 0.8
            elif rmssd < 40:
                stress_score = 0.6
...
class FitbitBackgroundSync:
    """Background service for continuous Fitbit data sync with eventlet"""
